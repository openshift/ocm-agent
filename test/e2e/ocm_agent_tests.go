...
var _ = Describe("ocm-agent", Ordered, func() {
	...
	BeforeAll(func(ctx context.Context) {
		...
		By("Setup: Setting up clients and prerequisites for tests")
		...
		if err != nil {
			Fail("Failed to add ocmagent to scheme")
		}

		err = appsv1.AddToScheme(k8sClient.Scheme())
		if err != nil {
			Fail("Failed to add deployment to scheme")
		}

		err = corev1.AddToScheme(k8sClient.Scheme())
		if err != nil {
			Fail("Failed to add service to scheme")
		}
		...
		By("Setup: Getting OCM configuration from ocm-agent configmap")
		...
		By("Setup: Getting real external cluster ID from configmap")
		if err == nil {
			if configMap.Data[clusterIDKey] != "" {
				externalClusterID = configMap.Data[clusterIDKey]
				By(fmt.Sprintf("Setup: External cluster ID: %s", externalClusterID))
			}
		}
		...
		By("Setup: Creating OCM connection and getting internal cluster ID")
		...
		if err != nil {
			GinkgoWriter.Printf("Skipping test: Failed to create OCM connection. Error: %v\n", err)
			Skip(fmt.Sprintf("Failed to create OCM connection: %v. This may be expected in some test environments.", err))
		}
		...
		if err != nil {
			...
			GinkgoWriter.Printf("Skipping test: Failed to get internal cluster ID. Error: %v\n", err)
			Skip(fmt.Sprintf("Failed to get internal cluster ID: %v. This may be expected if cluster is not registered in OCM.", err))
		}
		Expect(internalClusterID).ShouldNot(BeEmpty(), "internal cluster ID should not be empty")

		By("Setup: Creating network policy to allow traffic from all namespaces")
	...
	It("OcmAgentCommon - Testing basic deployment", func(ctx context.Context) {
		By("Step 1: Verifying that the namespace exists")
		err := client.Get(ctx, namespace, "", &corev1.Namespace{})
		Expect(err).Should(BeNil(), "namespace %s not found", namespace)

		By("Step 2: Verifying that the deployment exists")
	...
	It("OcmAgentCommon - Testing common ocm-agent tests", func(ctx context.Context) {
		By("Step 1: Listing ocm-agent pods for testing")
		...
		By("Step 2: Verifying ocm-agent service is ready")
		...
		By("Step 3: Testing health check endpoints")
		resp, err := httpClient.Get(fmt.Sprintf("%s/livez", ocmAgentURL))
		if err == nil {
		...
		By("Step 4: Verifying invalid endpoint returns 4xx error")
		...
		By("Step 5: Testing limited support reasons full workflow")
		...
		By("Step 5a: Creating limited support reason via OCM client")
		err = ocmClient.SendLimitedSupport(externalClusterID, lsReason)
		if err != nil {
			GinkgoWriter.Printf("Skipping test: Failed to create limited support reason. Error: %v\n", err)
			Skip(fmt.Sprintf("Failed to create limited support reason: %v. This may be expected if cluster doesn't support limited support or lacks permissions.", err))
		}

		// Since SendLimitedSupport doesn't return the ID, we have to find it.
		By("Step 5b: Finding the created limited support reason to get its ID")
		var limitedSupportReasonID string
		reasons, err := ocmClient.GetLimitedSupportReasons(externalClusterID)
		if err != nil {
			Fail(fmt.Sprintf("Failed to get limited support reasons after creating one: %v", err))
		}
		...
		By("Step 5c: Testing limited support reasons retrieval through ocm-agent")
		...
		if err != nil {
			GinkgoWriter.Printf("Skipping test: Failed to retrieve limited support reasons. Error: %v\n", err)
			Skip(fmt.Sprintf("Failed to retrieve limited support reasons via ocm-agent: %v. This may be expected if the agent doesn't proxy this endpoint.", err))
		} else {
		...
		By("Step 5d: Testing specific limited support reason retrieval")
		...
		if err != nil {
			GinkgoWriter.Printf("Skipping test: Failed to retrieve specific limited support reason. Error: %v\n", err)
			Skip(fmt.Sprintf("Failed to retrieve specific limited support reason via ocm-agent: %v. This may be expected if the agent doesn't proxy this endpoint.", err))
		} else {
			Expect(resp.StatusCode).Should(BeElementOf([]int{http.StatusOK, http.StatusNotFound, http.StatusUnauthorized}),
	...
	It("OcmAgentClassic - Testing alert processing for classic mode", func(ctx context.Context) {
		By("Step 1: Creating and managing default ManagedNotification")
		...
		By("Step 2: Verifying HTTP GET is not supported on alertmanager-receiver")
		...
		By("Step 3: Sending service log for a firing alert")
		...
		By("Step 4: Verifying alert has notification template associated")
	...
	It("OcmAgentHCP - Testing ocm-agent tests in fleet mode", func(ctx context.Context) {
		...
		var (
			...
			mFleetNotificationAuditWebhookErrorTemplate = &oav1alpha1.ManagedFleetNotification{
			...
			mFleetNotificationOidcDeletedTemplate = &oav1alpha1.ManagedFleetNotification{
				TypeMeta: metav1.TypeMeta{
					APIVersion: "ocmagent.managed.openshift.io/v1alpha1",
					Kind:       "ManagedFleetNotification",
				},
		...
		if err != nil {
			GinkgoWriter.Printf("Creating fleet mode deployment since it doesn't exist")
			...
			By("Setup: Creating OCM Agent Fleet mode deployment")
		...
		if err != nil {
			GinkgoWriter.Printf("Creating fleet mode service since it doesn't exist")
			...
			By("Setup: Creating OCM Agent Fleet mode service")
		...
		resp, err = httpClient.Get(fmt.Sprintf("%s/readyz", ocmAgentFleetURL))
		if err == nil {
			Expect(resp.StatusCode).Should(Equal(http.StatusOK), "readyz endpoint failed")
			resp.Body.Close()
		}
		...
		By("Step 2: Creating test ManagedFleetNotification templates")
	...
	AfterAll(func(ctx context.Context) {
		...
		By("Cleanup: Removing network policy")
		...
		By("Cleanup: Removing fleet-mode deployment")
		...
		By("Cleanup: Removing fleet-mode service")
