...
usage() {
    ...
    Example:
    $0 -s firing -n LoggingVolumeFillingUpNotificationSRE -m true -n LoggingVolumeFillingUp --start-date $(date -u +%Y-%m-%d) --end-date 0001-01-01
EOF
}

VALID_ARGS=$(getopt -o s:n:m:t:h --long alert-status:,alert-name:,send-managed-notification:,notification-template:,start-date:,end-date:,help -n $(basename $0) -- "$@")
if [[ $? -ne 0 ]]; then
    exit 1;
fi
