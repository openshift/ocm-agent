...
func (h *WebhookReceiverHandler) processAlert(alert template.Alert, mnl *oav1alpha1.ManagedNotificationList, firing bool) error {
	...
	if !canBeSent {
		if firing {
			...
			metrics.ResetResponseMetricFailure(config.ServiceLogService, notification.Name)
		} else {
	...
	if slerr != nil {
		...
		metrics.SetResponseMetricFailure(config.ServiceLogService, notification.Name)
